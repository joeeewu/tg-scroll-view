...
export default class Scroller {
  _callback: ScrollerCallback = noop;
  ...
  _isGesturing = false;
  _isDragging = false;
  _isDecelerating: number | boolean = false;
  _isAnimating: number | boolean = false;
  _clientLeft = 0;
  _clientTop = 0;
  _clientWidth = 0;
  _clientHeight = 0;
  ...
  _snapWidth = 100;
  _snapHeight = 100;
  _refreshHeight: number | null = null;
  _refreshActive = false;
  _refreshActivate: (() => void) | null = null;
  _refreshDeactivate: (() => void) | null = null;
  _refreshStart: (() => void) | null = null;
  _zoomLevel = 1;
  _scrollLeft = 0;
  _scrollTop = 0;
  _maxScrollLeft = 0;
  ...
  _lastTouchLeft = null;
  _lastTouchTop = null;
  _lastTouchMove: number | null = null;
  _positions: number[] = [];
  _minDecelerationScrollLeft = 0;
  _minDecelerationScrollTop = 0;
  _maxDecelerationScrollLeft = 0;
  _maxDecelerationScrollTop = 0;
  _decelerationVelocityX = 0;
  _decelerationVelocityY = 0;

  _zoomComplete: (() => void) | null = null;
  _interruptedAnimation = false;
  _initialTouchLeft = 0;
  _initialTouchTop = 0;
  _zoomLevelStart = 0;
  _lastScale = 0;
   ...
   */
  setDimensions(clientWidth: number | null, clientHeight: number | null, contentWidth: number | null, contentHeight: number | null) {
   ...
   */
  zoomTo(level: number, animate: boolean, originLeft: number | null, originTop: number | null, callback: () => void | null) {
    if (!this.options.zooming) {
      warn('Zooming is not enabled!');;
    }
  ...
  _publish(left: number, top: number, zoom = 1, animate = false) {
    ...
    if (animate && this.options.animating) {
      ...
      const step = (percent: number, _now: number, render: any) => {
        if (render) {
  ...
  _computeScrollMax(zoomLevel?: number) {
    if (zoomLevel == null) {
  ...
  _startDeceleration(_timeStamp: number) {
    if (this.options.paging) {
    ...
    // Wrap class method
    const step = (_percent: number, _now: number, render: any) => {
  ...
  _stepThroughDeceleration(render: any) {
    //
    ...
    if (!this.options.bouncing) {
      ...
      const scrollTopFixed = Math.max(Math.min(this._maxDecelerationScrollTop, scrollTop), this._minDecelerationScrollTop);
